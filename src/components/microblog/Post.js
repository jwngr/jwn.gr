  ...
  if (includeDateLink) {
    dateContent = (
      <p className="post-date">
        <Link to={`/microblog/${dateKey}`}>{getFormattedDate(dateKey)}</Link>
      </p>
    );
  } else {
    dateContent = <p className="post-date post-date-red">{getFormattedDate(dateKey)}</p>;
  }
  ...
  if (includeLocationLink) {
    locationContent = (
      <p className="post-location">
        <Link to={`/microblog/locations/${locationId}`}>{location}</Link>
      </p>
    );
  } else {
    locationContent = <p className="post-location">{location}</p>;
  }
  ...
  return (
    <div className="post">
      <MediaQuery maxWidth={750}>
        {dateContent}
        {locationContent}
        <img alt={dateKey} src={require(`../../images/microblog/posts/${dateKey}.jpg`)} />
